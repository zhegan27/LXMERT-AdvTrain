...
class NLVR2TorchDataset(Dataset):
    def __init__(self, dataset: NLVR2Dataset):
        ...
        if 'train' in dataset.splits:
            img_data.extend(load_obj_tsv('/img/train_obj36.tsv', topk=topk))
        if 'valid' in dataset.splits:
            img_data.extend(load_obj_tsv('/img/valid_obj36.tsv', topk=topk))
        if 'test' in dataset.name:
            img_data.extend(load_obj_tsv('/img/test_obj36.tsv', topk=topk))
        self.imgid2img = {}
